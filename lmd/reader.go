...
func ReadAFile(file_to_read string) string {
    ...
    // if "-" is passed as the file to read, read in from stdin instead.
...
// ImportIncludedFiles. included files need to be imported into the primary contents string. this function manages
...
// ParseTemplateToFindParameters. paramaters may be passed to the parser either separately from the
...
// UnmarshallParameters. yaml and json paramaters must be unmarshalled into the paramaters map. this function
...
// MergeParameters. this is a convenience function which will merge two hash maps. any conflicting parameters
